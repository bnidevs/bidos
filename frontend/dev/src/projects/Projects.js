...
function Switcher() {
  ...
  return (
    <span className="flex">
      <input
        id="public_switcher"
        className="switcher left_switcher no_margin"
        type="radio"
        name="vis"
        checked
      ></input>
      ...
      <input
        id="private_switcher"
        className="switcher right_switcher no_margin"
        type="radio"
        name="vis"
        onClick={check_logged_in}
      ></input>
...
function ProjectsList(props) {
  ...
  function SearchField(props) {
    ...
    const updateVal = async () => {
      ...
      setFiltered(
        fuzzysort
          .go(val, props.projList, {
            keys: ["project_name", "tagline"],
            all: true,
            threshold: -500,
          })
          .map((elem) => elem.obj)
      ...
      <div className="project_list">
        {filtered.map((project) => (
          <ProjectCard
            ...
            pool={currency_format.format(project.project_pool)}
          />
        ))}
      </div>
    </>
...
export { ProjectsPage, ProjectCard };
